...
For communication between the `C++` program that runs the audio generation and the `python` program that runs the visualisation, a few different options can be used. Two prominent options feature either embedded memory sharing, which is very difficult to implement and using a socket to share data via the internal network of the RPi. The latter option is chosen as its use is well documented and easy to implement. The `<socket.h>` library should be included and with this the local network can be accessed and data can be sent. In this case the whole buffer that also gets sent to the audio card, gets sent to the `python` program, in addition to the display setting that passed from the ESP through the `C++` program to the `python` program.
...
## Implementation
To integrate all the features on the RPi, the structure illustrated in the Nassi-Shneiderman diagram in [](#fig-nassi-shneider-rpi) is implemented in `C++`.
...
The full code for this implementation is given in ()[#code-full].

```{code}
:label: code-full
:caption: The full implementation of 
```

## Method
To test the functionalities listed in the [SP overview](#chapter-sp-overview)
